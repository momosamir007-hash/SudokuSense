...
# ─── تحميل نموذج الذكاء الاصطناعي (محدث للنموذج الجديد) ───
@st.cache_resource
def load_ai_model():
    model_path = 'model.keras'
    if os.path.exists(model_path):
        try:
            # تحميل النموذج الجديد بالكامل مع تجاهل إعدادات التدريب لمنع التعارض
        ...
        st.error("⚠️ ملف النموذج (model.keras) غير موجود! تأكد من رفعه للمستودع.")
        return None
...
with tab_img:
    ...
    if uploaded:
        ...
        if model is None:
            st.warning("⚠️ لا يمكن استخراج الأرقام بدون نموذج الذكاء الاصطناعي.")
            ...
            if st.button("🔍 استخراج الأرقام", type="primary", use_container_width=True, key="btn_extract"):
                ...
                with st.spinner("جاري المعالجة وقراءة الأرقام …"):
